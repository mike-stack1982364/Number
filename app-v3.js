const SETTINGS_KEY = "numberPattern.settings.v3";
const HISTORY_KEY = "numberPattern.history.v3";
...
function handleDocumentClick(event) {
  ...
  if (!matchButton || state.submitted || matchButton.disabled) return;
...
function submitResponse() {
  if (state.submitted || els.submit.disabled) return;
...
function requestNextTrial() {
  ...
  const delay = state.settings.trialDelayMs;
  if (delay > 0) {
    state.delayTimer = window.setTimeout(advanceTrial, delay);
  } else {
    advanceTrial();
  }
}
...
function finishSession() {
  ...
  els.summaryGrid.innerHTML = `<article><span>Pattern n-back</span><strong>${accuracy}%</strong></article><article><span>Scored comparisons</span><strong>${scoredComparisons}</strong></article>`;
  ...
  els.historyList.innerHTML = state.history.length
    ? state.history.map((record) => `
      <article class="history-card">
        ...
        <p>Pattern n-back ${record.accuracy}% · ${record.scoredComparisons} scored comparisons</p>
      </article>
    `).join("")
