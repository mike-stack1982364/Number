const SETTINGS_KEY = "numberPattern.settings.v4";
const HISTORY_KEY = "numberPattern.history.v4";
...
const els = {
  ...
  customTrialCount: $("#custom-trial-count"),
  spoken: $("#spoken-premises"),
  speechRate: $("#speech-rate"),
  trialDelay: $("#trial-delay"),
...
const state = {
  settings: loadObject(SETTINGS_KEY, DEFAULTS),
...
function handleDocumentClick(event) {
  ...
  if (!matchButton || state.submitted || matchButton.disabled || state.advancing) return;
...
function setTrialCountUi() {
  if (state.settings.infinite) {
    els.trialCount.value = "infinite";
    els.customTrialCount.hidden = true;
    return;
  }
...
function syncCustomTrialInput() {
  ...
  if (custom) {
    if (!els.customTrialCount.value) {
      els.customTrialCount.value = String(state.settings.trialCount || 24);
    }
    els.customTrialCount.focus();
  }
}
...
function readSettings() {
  ...
  if (!infinite) {
    trialCount = els.trialCount.value === "custom"
      ? Number(els.customTrialCount.value)
      : Number(els.trialCount.value);
    if (!Number.isInteger(trialCount) || trialCount < 1) {
      throw new Error("Enter a whole number of trials greater than zero.");
    }
  }
  ...
  return {
    ...
    trialCount,
    infinite,
    spoken: els.spoken.checked,
    speechRate: Number(els.speechRate.value),
    trialDelayMs: Number(els.trialDelay.value),
  };
}
...
function ensureTrial(index) {
  while (state.trials.length <= index) {
    state.trials.push(makeTrial(state.trials.length));
  }
}

function makeTrial(index) {
  const nBack = state.settings.nBack;
  const canMatch = index >= nBack;
  const targetMatch = canMatch && Math.random() < 0.34;
  let rule;

  if (targetMatch) {
    rule = RULES.find((candidate) => candidate.id === state.trials[index - nBack].ruleId);
  } else {
    const forbidden = canMatch
      ? state.trials[index - nBack].ruleId
      : state.trials.at(-1)?.ruleId;
  ...
  return {
    ruleId: rule.id,
    numbers: rule.make(start),
    match: canMatch ? rule.id === state.trials[index - nBack].ruleId : null,
  };
}
...
function renderTrial() {
  ...
  els.trialLabel.textContent = state.settings.infinite
    ? `Trial ${state.index + 1} · Infinite`
    : `Trial ${state.index + 1} / ${state.settings.trialCount}`;
  els.progressBar.style.width = state.settings.infinite
    ? "100%"
    : `${((state.index + 1) / state.settings.trialCount) * 100}%`;
  els.nBadge.textContent = `${state.settings.nBack}-back`;
  els.scoreBadge.textContent = liveScore();

  els.series.replaceChildren(
    ...trial.numbers.flatMap((number, index) => {
  ...
  els.submit.hidden = warmup;
  els.submit.disabled = true;
  els.next.hidden = !warmup;
  els.next.disabled = false;
  els.next.textContent = state.settings.infinite
    ? "Next trial"
    : state.index === state.settings.trialCount - 1
      ? "Finish session"
      : "Next trial";

  speakSeries(trial.numbers);
  scheduleTrialExpiry();
}

function scheduleTrialExpiry() {
  const duration = state.settings.trialDelayMs;
  if (!state.active || duration <= 0) return;
...
function handleTrialExpiry() {
  ...
  const trial = state.trials[state.index];
  if (trial.match !== null && !state.submitted) {
    state.results.push({
      correct: false,
      omitted: true,
      responseTimeMs: performance.now() - state.trialStartedAt,
    });
  }
  cancelSpeech();
  advanceTrial();
...
function submitResponse() {
  if (state.submitted || els.submit.disabled || state.advancing) return;
  state.submitted = true;
  cancelSpeech();

  const trial = state.trials[state.index];
  const correct = state.selectedMatch === trial.match;
  state.results.push({
    correct,
    omitted: false,
    responseTimeMs: performance.now() - state.trialStartedAt,
  ...
  for (const button of els.memoryCard.querySelectorAll("button[data-match]")) {
    const value = button.dataset.match === "true";
    button.disabled = true;
    button.classList.remove("selected");
    if (value === trial.match) button.classList.add("correct");
    else if (value === state.selectedMatch) button.classList.add("wrong");
  }
...
function advanceTrial() {
  ...
  if (!state.settings.infinite && state.index >= state.settings.trialCount - 1) {
    finishSession();
    return;
  }

  state.index += 1;
  ensureTrial(state.index);
  renderTrial();
}
...
function endSession() {
  ...
  const confirmed = window.confirm(
    state.settings.infinite
      ? "End the infinite session and save completed scored comparisons?"
      : "End this session now and save completed scored comparisons?",
  );
  if (!confirmed) return;

  clearTrialTimer();
  if (state.results.length > 0) finishSession();
  else {
    state.active = false;
    showView("play");
  }
...
function finishSession() {
  ...
  const scoredComparisons = state.results.length;
  const omissions = state.results.filter((result) => result.omitted).length;
  const presentedTrials = Math.min(state.index + 1, state.trials.length);
  const record = {
    date: new Date().toISOString(),
    nBack: state.settings.nBack,
    trials: presentedTrials,
    scoredComparisons,
    omissions,
    infinite: state.settings.infinite,
    accuracy,
    spoken: state.settings.spoken,
    speechRate: state.settings.speechRate,
    trialDelayMs: state.settings.trialDelayMs,
  };
  ...
  els.summaryHeading.textContent = accuracy >= 85
    ? "Pattern memory held"
    : accuracy >= 70
      ? "Pattern memory is forming"
      : "Pattern discrimination needs reinforcement";
  ...
  els.summaryNote.textContent = `Presented ${presentedTrials} trial${presentedTrials === 1 ? "" : "s"} at ${state.settings.nBack}-back${state.settings.infinite ? " in infinite mode" : ""}. Warm-up trials were not scored.`;
  showView("summary");
}

function clearTrialTimer() {
  if (state.trialTimer) {
    window.clearTimeout(state.trialTimer);
    state.trialTimer = 0;
  }
}
...
function liveScore() {
  ...
  return `Score ${percentage(state.results.map((result) => result.correct))}%`;
}
  ...
  els.historyList.innerHTML = state.history.length
    ? state.history.map((record) => `
      <article class="history-card">
        <header>
          <div><b>${new Date(record.date).toLocaleString()}</b><p>${record.nBack}-back · ${record.trials} presented trials${record.infinite ? " · infinite mode" : ""}${record.spoken ? " · auditory" : ""}</p></div>
          <strong>${record.accuracy}%</strong>
        </header>
        <p>Pattern n-back ${record.accuracy}% · ${record.scoredComparisons} scored comparisons · ${record.omissions || 0} omissions</p>
      </article>
    `).join("")
...
function primeSpeech() {
  ...
  try {
    ...
    // The session-start gesture remains the fallback activation.
  }
}

function speakSeries(numbers) {
  cancelSpeech();
  if (!state.settings.spoken || !window.speechSynthesis || !window.SpeechSynthesisUtterance) return;
  ...
  try {
    ...
    // Visual presentation remains available.
  }
}
...
document.addEventListener("visibilitychange", () => {
  if (document.hidden) {
    clearTrialTimer();
    cancelSpeech();
  } else if (state.active && !state.trialTimer) {
    state.trialStartedAt = performance.now();
    scheduleTrialExpiry();
  }
});